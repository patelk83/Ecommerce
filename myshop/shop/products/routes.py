...
def deletebrand(id):
    ...
    if request.method== "POST"or request.method =="GET":
        ...
        db.session.commit()
        flash(f'The brand{brand.name} deleted ','success')
...
def deletecategory(id):
    ...
    if request.method== "POST" or request.method =="GET":
        ...
        db.session.commit()
        flash(f'The category{category.name} deleted ','success')
        return redirect(url_for('admin'))
...
def updateproduct(id):
    ...
    if request.method== "POST":
        ...
        product.bran_id= brand
        product.category_id=category
        product.colors= form.colors
        product.desc= form.description.data
        if request.files.get('image_1'):
            try:
                os.unlink(os.path.join(current_app.root_oath,"static/images"+product.image_1))
                product.image_1 = photos.save(request.files.get('image_1'), name=secrets.token_hex(10) + ".")
            except:
                product.image_1 = photos.save(request.files.get('image_1'), name=secrets.token_hex(10) + ".")
        if request.files.get('image_2'):
            try:
                os.unlink(os.path.join(current_app.root_oath, "static/images" + product.image_2))
                product.image_2 = photos.save(request.files.get('image_2'), name=secrets.token_hex(10) + ".")
            except:
                product.image_2 = photos.save(request.files.get('image_2'), name=secrets.token_hex(10) + ".")
        if request.files.get('image_3'):
            try:
                os.unlink(os.path.join(current_app.root_oath, "static/images" + product.image_3))
                product.image_3 = photos.save(request.files.get('image_3'), name=secrets.token_hex(10) + ".")
            except:
                product.image_3 = photos.save(request.files.get('image_3'), name=secrets.token_hex(10) + ".")

        db.session.commit()
        flash('Product has been success','success')
...
def deleteproduct(id):
    ...
    if request.method== "POST"or request.method =="GET":
        ...
        db.session.commit()
        flash(f'The brand {product.name} deleted ','success')
        return redirect(url_for('admin'))
    flash(f'The brand {product.name} can not be deleted ', 'warning')
    return redirect(url_for('admin'))
